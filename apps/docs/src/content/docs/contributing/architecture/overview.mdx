...
- **Factory-based task creation** — Most tasks use `createSimpleFileTask`, `createJsonMergeTask`, or `createFileTask` to eliminate boilerplate
...
flowchart TD
    ...
    Core --> Utils["utils/<br/>fs, diff, logger (picocolors), pkg (pkg-types)"]
