...
import type { FileDiff, Framework } from '@xtarterize/core'
import {
	applyTasks,
	detectProject,
	logger,
...
async function resolveAmbiguousFramework(): Promise<Framework> {
...
export const initCommand = defineCommand({
	...
	async run({ args }) {
		...
		if (profile.framework === null) {
			if (!quiet) s.stop()
			const pkg = await import('@xtarterize/core').then((m) =>
				m.readPackageJson(cwd),
			)
