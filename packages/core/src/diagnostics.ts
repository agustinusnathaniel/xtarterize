...
export async function checkToolInstalled(tool: string, cwd: string): Promise<boolean> {
  try {
...
export async function runConflictChecks(cwd: string): Promise<DiagnosticCheck[]> {
  ...
  if (hasBiome && hasEslint) {
    checks.push({
      ...
      status: 'warn',
      message: 'Both Biome and ESLint are configured. Consider using one as primary.',
    })
  }
  ...
  if (hasBiome && hasPrettier) {
    checks.push({
      name: 'Conflicting tools',
      status: 'warn',
      message: 'Both Biome and Prettier are configured. Biome includes formatting — Prettier may be redundant.',
    })
  }
...
export async function runToolInstallationChecks(cwd: string): Promise<DiagnosticCheck[]> {
  const checks: DiagnosticCheck[] = []
  const pkg = await readPackageJson(cwd)
  if (!pkg) return checks
  ...
  const toolsToCheck: { name: string; dep: string; required: boolean; cmd?: string }[] = [
    ...
    { name: 'Commitlint', dep: '@commitlint/cli', required: false, cmd: 'commitlint' },
