...
describe('biomeTask', () => {
  ...
  it('returns new on clean fixture', async () => {
    const profile = await detectProject(path.join(fixtures, 'react-vite-tailwind'))
    const status = await biomeTask.check(path.join(fixtures, 'react-vite-tailwind'), profile)
    expect(status).toBe('new')
  })

  it('dryRun returns diffs', async () => {
    const profile = await detectProject(path.join(fixtures, 'react-vite-tailwind'))
    const diffs = await biomeTask.dryRun(path.join(fixtures, 'react-vite-tailwind'), profile)
